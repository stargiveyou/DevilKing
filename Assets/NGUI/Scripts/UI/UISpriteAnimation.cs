...
public class UISpriteAnimation : MonoBehaviour
{
	...
	void Start () {
        RebuildSpriteList();
        isTuto = UnityEngine.SceneManagement.SceneManager.GetActiveScene().name.Equals("Tutorial") && gameObject.name.Equals("Player");
        ...
        if (mActive && mSpriteNames.Count > 1 && Application.isPlaying && mFPS > 0f)
        {
            ...
            if (rate < mDelta)
            {
                ...
                if (mActive)
                {
                    ...
                    if (isTuto)
                    {
                        mSprite.transform.localScale = new Vector3(2.3f, 2.3f, 2.3f);
                    }
                }
            }
        }
